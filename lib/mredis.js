 ...
 * @throws {Error}
 */
var mredis = function(_config) {
  this.config = _config !== undefined ? 
  _config : {
    debug: true,
  ...
  //
  // Check mredis config if exists
  // 
  for (var curHost in this.config.hosts) {
    if (!this.config.hosts.hasOwnProperty(curHost))
      continue;
    this.redisHosts.push(curHost);

    if (typeof this.config.hosts[curHost] !== 'object' ||
      (this.config.hosts[curHost] instanceof Array && !this.config.hosts[curHost].length)) {
      wrongHostsDeclaration = true;
      break;
    }

    if (!(this.config.hosts[curHost] instanceof Array) && (
        (this.config.hosts[curHost].ports === undefined || !this.config.hosts[curHost].ports) || (
          this.config.hosts[curHost].ports !== undefined &&
          this.config.hosts[curHost].ports instanceof Array &&
          !this.config.hosts[curHost].ports.length
        )
      )) {
      wrongHostsDeclaration = true;
      break;
 ...
 */
mredis.prototype.memcachedConnect = function(callback) {
  if (!this.memcached) {
 ...
 */
mredis.prototype.redisConnect = function(connectionData, callback) {
  var _this = this;
 ...
 */
mredis.prototype.getConnectionData = function(key, command) {
  var connectionData = {
    host: null,
    port: null
  };
 ...
 */
mredis.prototype.processKeyForMemcached = function(params, command) {
  switch (command) {
 ...
 */
mredis.prototype.processResultFromMemcached = function(value, command) {
 ...
 * @param  {Mixed} value
 */
mredis.prototype.applyUserCallback = function(userCallback, err, value) {
 ...
 * @return {Number}
 */
mredis.prototype.getRand = function(min_random, max_random) {
 ...
 * @return {Number}
 */
mredis.prototype.getCrc32 = function(str) {
 ...
 */
mredis.prototype.log = function(message, block) {
  if (this.debug) {
    console.log('[mredis' + (block ? ' ' + block : ' ') + '] --- ' + message);
  }
}
 ...
 */
mredis.prototype.exec = function(command, params, userCallback) {
  var _this = this;
